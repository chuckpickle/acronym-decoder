...
| `PopupComponent` → `src/app/popup/Popup.tsx` (search popup) | Child C | `devin/1788407742-react-popup` | done | 9 Vitest tests in `Popup.test.tsx` (25 total pass); typecheck + build pass. `openDefaultEmailAddress` duplicated into `src/app/popup/open-default-email.ts` (copy in `background.ts` to dedupe in cleanup) |
| Remove Angular artifacts (`*.module.ts`, `*.component.*`, `*.spec.ts`, `angular.json`, Karma, `@angular/*` deps) and verify extension loads | Managing session | `devin/1788408039-react-integration` | done | Merged Child A/B/C branches; deleted `angular.json`, `karma.conf.js`, `protractor.conf.js`, `tslint.json`, `e2e/`, `src/test.ts`, `src/tsconfig.{app,spec}.json`, `src/environments/`, `src/testing/spec-helpers.ts`, all `*.component.*` / `*.module.ts` / `*.spec.ts`; dropped `@angular/*`, `@ng-bootstrap`, `zone.js`, `core-js`, Karma/Jasmine/Protractor/tslint deps (added `lodash` explicitly — it was only transitive before). `openDefaultEmailAddress` deduped (lives in `src/app/popup/open-default-email.ts`). typecheck / test:ci (36 tests) / build pass; `dist/browser` loads as unpacked extension. |
...
- Scripts: `npm run build` (vite + webpack), `npm test` / `npm run test:ci`, `npm run typecheck` (strict TS; also aliased as `npm run lint`).
- TypeScript: root `tsconfig.json` is `strict` (`strictPropertyInitialization` off so the model classes stay as-is).
